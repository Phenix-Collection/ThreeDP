...
import com.chat.entity.InviteMessage;
...
public class ChatDBManager {
    ...
    private ChatDBManager(){
        dbHelper = DbOpenHelper.getInstance(Globals.getContext().getApplicationContext());
    }
    ...
    synchronized public List<InviteMessage> getMessagesList(){
        ...
        if(db.isOpen()){
            Cursor cursor = db.rawQuery("select * from " + InviteMessgeDao.TABLE_NAME + String.valueOf(" desc"),null);
            while(cursor.moveToNext()){
