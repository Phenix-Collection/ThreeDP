...
public class ForgetPwdActivity extends BaseActivity {
    ...
    @OnClick({R.id.tv_exit, R.id.id_send, R.id.id_ok})
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.id_send: // 发送验证码
                ...
                HttpRequest.instance().doPost(HttpRequest.create(WebUcenterApi.class).sendCode(phone, secretKey, "_forget"), new OnResultListener() {
                    @Override
                    public void onWebUiResult(WebMsg webMsg) {
                        if(webMsg.isSuccess()){
                            Toast.show(ForgetPwdActivity.this, "验证码已发送到您手机，请查收！", android.widget.Toast.LENGTH_LONG);
                            countDown();
                        } else {
                            sendCodeIsBusying = false;
                            webMsg.showMsg(ForgetPwdActivity.this);
                        }
                    }
                });
                break;
            case R.id.id_ok: // 确定
                ...
                if(pwd.length() < 6){
                    Toast.show(ForgetPwdActivity.this, "密码至少为6位以上字符！", android.widget.Toast.LENGTH_LONG);
                    return;
                }

                if(!pwd.equals(pwdNew)){
                    Toast.show(ForgetPwdActivity.this, "确认密码需与密码一致！", android.widget.Toast.LENGTH_LONG);
                    return;
                }

                String code = codeEdt.getText().toString();
                if(code.length() != 4){
                    Toast.show(ForgetPwdActivity.this, "请输入4位数的验证码！", android.widget.Toast.LENGTH_LONG);
                    return;
                }


                Loading.start(this, "修改密码中，请稍等...", false);
                HttpRequest.instance().doPost(HttpRequest.create(WebUcenterApi.class).forget(phone, pwd, code, MD5.md5(phone + pwd + Globals.APP_KEY)), new OnResultListener() {
                    @Override
                    public void onWebUiResult(WebMsg webMsg) {
                        Loading.stop();
                        if(webMsg.isSuccess()){
                            Toast.show(ForgetPwdActivity.this, "修改成功！", Toast.LENGTH_LONG);
